...
import { addCommentFormProps } from "@/utils/types";
...
const AddCommentForm = ({ articleId }: addCommentFormProps) => {
  ...
  const formSubmitHandler = async (e: React.FormEvent) => {
    e.preventDefault();
    if (text === "") {
      return toast.error(" please write some thing ");
    }
    try {
      setLoading(true);
  ...
  return (
    <form onSubmit={formSubmitHandler} className="mb-7">
      ...
      <button
        type="submit"
        className="bg-green-700 text-white mt-2 p-1 w-full text-xl rounded-lg hover:bg-green-900 transition"
      >
        {loading ? (
          <>
            <div
